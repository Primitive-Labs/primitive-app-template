...
primitive whoami                   # Check auth status
```
...
1. **Run `primitive guides list`** to discover available topics
2. **Identify which guides are relevant** to their feature from the list output
3. **Fetch those guides** with `primitive guides get <topic>`
4. **Recommend a data modeling approach** based on the guide content
5. **Outline the implementation steps** referencing specific patterns from the guides
6. **Write the code** following the patterns exactly
7. **Review automatically** per Step 4 above
